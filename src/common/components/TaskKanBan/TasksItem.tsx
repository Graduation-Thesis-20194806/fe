...
export const getPriorityIconByID = (id?: TaskListItemEntity.priority) => {
  let icon;
  switch (id) {
    case TaskListItemEntity.priority.IMMEDIATE:
        ...
        <Row justify="space-between">
          <Col span={20}>
            ...
            <Typography.Paragraph
              ...
              className="text-[10px]"
            >
              <div className="task-description" dangerouslySetInnerHTML={{ __html: item?.description ?? "<></>"}} />
            </Typography.Paragraph>
          </Col>
        </Row>
        <Row
        ...
        >
          <Col>
            <Space align="center" size="middle">
              <Tag>{formatDate(item?.deadline)}</Tag>
            </Space>
          </Col>
          <Col>
            <Space align="center">
              {getPriorityIconByID(item?.priority)}
              {item?.ProjectMember.user && (
                <Tooltip title={item.ProjectMember.user?.username}>
                  {item.ProjectMember.user?.avatar ? (
                    <Avatar
                      size="small"
                      src={getS3Link(item.ProjectMember.user.avatar)}
                    />
                  ) : (
                    <Avatar
                      icon={<UserOutlined />}

...
import { getStorage, removeStorage, setStorage } from "@/common/helpers/storage";
...
apiClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (
      ...
      const refreshToken = getStorage("refresh_token", "local");
      ...
      if (refreshToken) {
        try {
          ...
          setStorage('token', newAccessToken, 'local')
          setStorage('refresh_token', newRefreshToken, 'local')

          originalRequest.headers["Authorization"] = `Bearer ${newAccessToken}`;
          return apiClient(originalRequest);
        } catch (refreshError) {
        removeStorage('token', 'local');
        removeStorage('refresh_token', 'local')
